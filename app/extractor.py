...
from app.schemas import (
    ...
    QuestionItem,
    RequirementItem,
    RequirementQualityEnrichmentOutput,
    RewrittenItem,
    SpecOutput,
...
from app.quality import (
    build_enriched_item_fallback,
    coerce_quality_checks,
...
def _match_rewritten_item(
    raw: dict[str, Any],
    rewritten_items: list[RewrittenItem],
) -> RewrittenItem | None:
    rid = str(raw.get("id", "")).strip()
    by_id = {item.id: item for item in rewritten_items}
    if rid in by_id:
        return by_id[rid]
    ...
    raw_sources = {
        str(source_id).strip()
        for source_id in raw.get("source_units", [])
        if str(source_id).strip()
    }
    for item in rewritten_items:
        if raw_text and normalize_text(item.text) == raw_text:
            return item
        if raw_sources and raw_sources.intersection(set(item.source_units)):
            raw_type = str(raw.get("type", "")).strip()
            if not raw_type or raw_type == item.type:
                return item
    return None


def validate_requirement_quality_enrichment(
    payload: dict[str, Any],
    conversation_units: Iterable[ConversationUnit],
    rewritten_output: Stage3RewrittenOutput,
) -> RequirementQualityEnrichmentOutput:
    field_name, raw_items = _first_list_field(
        payload,
        (
            "enriched_items",
            "requirements",
    ...
    if raw_items is None:
        raise ExtractionError(
            "Requirement quality enrichment output must include list field `enriched_items`."
        )

    units = list(conversation_units)
    valid_unit_ids = {u.id for u in units}
    rewritten_items = list(rewritten_output.rewritten_items)
    enriched_by_id: dict[str, EnrichedRequirementItem] = {}

    for raw in raw_items:
        if not isinstance(raw, dict):
            continue
        base_item = _match_rewritten_item(raw, rewritten_items)
        if base_item is None:
            continue

        rtype = str(raw.get("type", "")).strip()
        if rtype not in REWRITE_ALLOWED_TYPES:
            rtype = base_item.type
        text = str(raw.get("text", "")).strip() or base_item.text
        source_units = _normalize_source_units(raw.get("source_units"), valid_unit_ids)
        if not source_units:
            source_units = _normalize_source_units(base_item.source_units, valid_unit_ids)

        evidence_spans = _clean_text_list(
            raw.get("evidence_spans")
            or raw.get("evidence")
            or raw.get("evidence_span")
        )
        fallback_item = build_enriched_item_fallback(base_item, units)
        if not evidence_spans:
            evidence_spans = fallback_item.evidence_spans
        acceptance_criteria = _clean_text_list(
            raw.get("acceptance_criteria")
            or raw.get("acceptanceCriteria")
            or raw.get("criteria")
        )
        if not acceptance_criteria:
            acceptance_criteria = fallback_item.acceptance_criteria or default_acceptance_criteria(text, rtype)

        quality_checks = coerce_quality_checks(
            raw.get("quality_checks"),
            requirement_text=text,
            source_units=source_units,
            evidence_spans=evidence_spans,
            acceptance_criteria=acceptance_criteria,
        )
        enriched_by_id[base_item.id] = EnrichedRequirementItem(
            id=base_item.id,
            type=rtype,
            text=text,
            source_units=source_units,
    ...
    for item in rewritten_items:
        if item.id not in enriched_by_id:
            enriched_by_id[item.id] = build_enriched_item_fallback(item, units)

    if raw_items and not enriched_by_id and rewritten_items:
        raise ExtractionError(
            f"Requirement quality enrichment field `{field_name}` did not match rewritten items."
        )

    return RequirementQualityEnrichmentOutput(
        enriched_items=[enriched_by_id[item.id] for item in rewritten_items if item.id in enriched_by_id]
    )


def build_requirement_quality_fallback(
    conversation_units: Iterable[ConversationUnit],
    rewritten_output: Stage3RewrittenOutput,
) -> RequirementQualityEnrichmentOutput:
    units = list(conversation_units)
    return RequirementQualityEnrichmentOutput(
        enriched_items=[
            build_enriched_item_fallback(item, units)
