...
The objective of v0.1.1 is to improve the practical quality of the generated requirements specification while keeping the system runnable locally. The specific goals are:
...
- keep the final specification readable while preserving verification signals for human review.
...
The v0.1.1 pipeline uses a source-unit decision prompt followed by deterministic construction and post-processing. The LLM produces source-grounded decisions, but the final specification is assembled by code. This design keeps the model focused on extraction and classification while deterministic components control the final schema, evidence links, and report structure.

### Verification and Post-processing

After deterministic construction, the pipeline enriches each requirement with evidence spans and acceptance criteria. MiniCheck is then used as the main verifier. Each generated requirement is treated as a claim, and the concatenated text of its referenced source units is treated as evidence. The verifier estimates whether the claim is supported by the evidence and records a requirement-level verdict and confidence value.
...
| Model policy | Base-model-oriented local configuration | Instruction-tuned model preference | More stable instruction following |
...
| Groundedness verification | No requirement-level verifier output | MiniCheck-based support probability and verdicts | Reviewers can identify weakly grounded requirements |
| Report output | Requirement draft and review details were less separated | Cleaner specification with separate review signals | More readable final specification |
| Post-processing | Limited cleanup | Confidence-aware cleanup for weak items, answered questions, privacy prohibitions, and future constraints | Reduces common local-model errors |

### Experimental Setup

Both versions were evaluated with the same Qwen3-4B instruction-tuned model. The model was held fixed so that the main result measures the pipeline change from v0.1.0 to v0.1.1 rather than a model upgrade.

The comparison used a combined 30-sample evaluation dataset covering booking, service, and operations-style conversations. To isolate pipeline differences, both v0.1.0 and v0.1.1 used the same model. The v0.1.0 generation pipeline itself remained unchanged.

An additional secondary model-policy experiment compares the v0.1.1 pipeline with a base model and an instruction-tuned model.
...
Because the semantic F1 metric is lightweight and project-specific, it should be interpreted as an internal comparison metric rather than a general benchmark score. MiniCheck verification results are used mainly for v0.1.1 reviewability analysis, not as the primary v0.1.0-vs-v0.1.1 extraction score.
...
Because the course includes fine-tuning and instruction tuning topics, an additional v0.1.1-only experiment compared a base-style Qwen3-4B model with an instruction-tuned Qwen3-4B model. The comparison focuses on base versus instruction-tuned behavior. The pipeline and 30-sample dataset were fixed.

| Model | Semantic Macro F1 | FR F1 | NFR F1 | CON F1 | Semantic Unmatched | Source-aware Unmatched | Latency | LLM Calls |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Qwen3-4B base | 0.829 | 0.954 | 0.794 | 0.741 | 0.057 | 0.086 | 24.30s | 4.93 |
| Qwen3-4B instruct | 0.827 | 0.938 | 0.814 | 0.731 | 0.029 | 0.066 | 23.37s | 4.83 |
...
The related work supports this direction. NLP4RE research emphasizes ambiguity, incompleteness, and the need for semantic processing in requirements engineering. This motivated source-unit segmentation and category-specific evaluation. LLM-based software specification research motivated controlled pipeline evaluation under a fixed model. Generative LLM work in requirements engineering emphasizes prompt design and output control, which motivated the controlled source-unit decision schema and the instruction-tuned model policy. MiniCheck directly motivated evidence-based verification: in v0.1.1, generated requirements are treated as claims and checked against source evidence before review verdicts are produced. RAGAS motivated separating evaluation dimensions rather than relying on one global score.

The comparison also shows an important trade-off. v0.1.1 predicts more requirements than v0.1.0, increasing the total predicted requirement count from 106 to 137. This improves recall and category coverage, especially for constraints, while the semantic unmatched and source-aware unmatched rates remain low. For a requirements engineering assistant, this trade-off is acceptable if the generated specification is treated as a draft for human review and accompanied by verification signals. However, it also means that the system should not be presented as a fully automatic replacement for human requirements analysis.
